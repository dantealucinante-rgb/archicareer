...
export async function POST(request: Request) {
    try {
        ...
        if (typeof body.marketing_emails !== "boolean") {
            return NextResponse.json({ error: "marketing_emails must be a boolean" }, { status: 400 });
        }
        ...
        const { data, error } = await updateProfile(profile.id, { marketing_emails: body.marketing_emails });
        if (error || !data) {
            return NextResponse.json({ error: "Unable to save settings" }, { status: 400 });
        }

        return NextResponse.json({ settings: { marketing_emails: data.marketing_emails } });
    } catch {
