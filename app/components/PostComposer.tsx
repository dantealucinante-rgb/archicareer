...
export default function PostComposer({ onCreated }: { onCreated: (post: FeedPost) => void }) {
    ...
    return <form onSubmit={submit} className="surface p-5 sm:p-6"><div className="flex gap-3"><textarea value={content} onChange={(event) => setContent(event.target.value)} maxLength={1000} rows={3} className="field-input resize-y text-sm" placeholder="What are you thinking about in architecture?" /><div className="hidden w-28 shrink-0 sm:block"><label htmlFor="post-image" className="flex h-full cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-line p-3 text-center font-mono text-[9px] uppercase tracking-widest text-graphite hover:border-redline hover:text-redline">{file ? "Change image" : "Add image"}<input id="post-image" type="file" accept="image/jpeg,image/png,image/webp,image/gif" className="sr-only" onChange={(event) => selectImage(event.target.files?.[0] ?? null)} /></label></div></div>{previewUrl && <div className="relative mt-3 aspect-[4/3] overflow-hidden rounded-2xl border border-line"><NextImage src={previewUrl} alt="Selected post preview" fill unoptimized className="object-cover" /><button type="button" onClick={() => selectImage(null)} className="absolute right-2 top-2 rounded-full bg-ink/80 px-3 py-1 font-mono text-[9px] uppercase tracking-widest text-paper">Remove</button></div>}<div className="mt-3 flex items-center justify-between gap-3"><label htmlFor="post-image-mobile" className="font-mono text-[10px] uppercase tracking-widest text-graphite sm:hidden">{file ? "Change image" : "Add image"}<input id="post-image-mobile" type="file" accept="image/jpeg,image/png,image/webp,image/gif" className="sr-only" onChange={(event) => selectImage(event.target.files?.[0] ?? null)} /></label><p aria-live="polite" className="font-mono text-[10px] uppercase tracking-widest text-redline">{message}</p><button type="submit" disabled={saving || (!content.trim() && !file)} className="rounded-full border border-ink bg-ink px-5 py-2.5 font-mono text-[10px] uppercase tracking-widest text-paper hover:bg-redline disabled:cursor-not-allowed disabled:opacity-50">{saving ? "Publishing..." : "Publish"}</button></div></form>;
}
