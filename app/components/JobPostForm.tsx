...
export default function JobPostForm() {
        ...
        try {
            const response = await fetch("/api/jobs", {
                ...
                body: JSON.stringify({
                    ...
                    type,
                    description,
                    apply_link_or_email: applyContact || undefined,
                }),
            });
            const payload = await response.json().catch(() => ({}));
            if (!response.ok) throw new Error(payload.error ?? "Unable to submit posting");
            setTitle(""); setType("internship"); setApplyContact(""); setDescription("");
            setStatus("idle");
    ...
    return (
        <form className="mt-6 space-y-4" onSubmit={submit}>
            ...
            <div><label className={label}>Apply Link or Email</label><input value={applyContact} onChange={(event) => setApplyContact(event.target.value)} className={input} /></div>
